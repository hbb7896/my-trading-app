...
if not df.empty:
    ...
    with tab1:
        ...
        # 2. 코스피 데이터 (FinanceDataReader로 교체)
        try:
            ...
            combined_chart = alt.layer(my_chart, kospi_chart).resolve_scale(y='independent')
    ...
    # === TAB 3: 연도별 분석 (기존 유지 + 색상 에러 방지용 matplotlib 필수) ===
    with tab3:
    ...
    with tab5:
        ...
        if not losses.empty:
            ...
            with c2:
                ...
                if "실패" in found_name:
                    st.warning(f"'{target_name}' 코드를 못 찾았습니다. 정확한 이름을 입력해주세요.")
                else:
