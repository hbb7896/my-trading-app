...
if not df.empty:
    ...
    # === TAB 7: 심플 자금 관리 ===
    with tab7:
        st.subheader("⚖️ 심플 자금 관리 (50-35-15 Rule)"); st.markdown("**\"복잡한 건 질색! 50% - 35% - 15% 비율만 딱 알려줍니다.\"**");
        with st.container(border=True): col_in1, col_in2 = st.columns(2);
            with col_in1: total_money = st.number_input("💰 이번 종목 총 투입 예정금 (원)", value=2000000, step=100000)
            with col_in2: current_price = st.number_input("🎯 현재 주가 (1차 진입가)", value=70000, step=100)
        amt1 = total_money * 0.50; amt2 = total_money * 0.35; amt3 = total_money * 0.15; qty1 = int(amt1 / current_price) if current_price > 0 else 0; price2 = int(current_price * 1.02); qty2 = int(amt2 / price2) if price2 > 0 else 0; price3 = int(current_price * 1.05); qty3 = int(amt3 / price3) if price3 > 0 else 0; st.write(""); c1, c2, c3 = st.columns(3);
        with c1: st.success(f"**1️⃣ 1차 진입 (50%)**\n# {amt1:,.0f} 원\n* **{qty1:,} 주** 매수\n* (현재가 즉시 진입)")
        with c2: st.warning(f"**2️⃣ 2차 불타기 (35%)**\n# {amt2:,.0f} 원\n* **{qty2:,} 주** 매수\n* (수익률 +2% 시점)")
        with c3: st.error(f"**3️⃣ 3차 막타 (15%)**\n# {amt3:,.0f} 원\n* **{qty3:,} 주** 매수\n* (수익률 +5% 시점)")
